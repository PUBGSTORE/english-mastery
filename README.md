...
The sidebar (and the phone tab bar) has two areas: **📘 English** for language learning and **🧠 Knowledge** for learning about the world. The switch at the top of the sidebar flips between them; opening any Knowledge page flips automatically. Knowledge holds three sections: Daily Discovery, Get Fast Knowledge and Book explainer, plus a Knowledge home with today's topics, streak and recent items.

## 🧠 Daily Discovery

Every day three topics are picked (seeded by the date) from `data/discover-topics.json`: 37 fields from economics, physics and neuroscience to law, music and space technology, 222 curated topics each with a one-line hook. Type any genre ("neuroscience") and the tutor picks a topic inside it, or any specific topic ("how inflation works"), or tap 🎲 Surprise me. One DeepSeek call (about ₹0.30) returns: a 10-year-old explanation, the real explanation, how it works step by step, three examples, three real-life situations with what to do differently, why it is useful, five apply-this-week actions (checkboxes), a link to security engineering, a one-line mental model, misconceptions, what experts know, facts, history, a 4-question quiz with scoring, a teach-it-back box graded for understanding and English, terms in English/हिन्दी/ગુજરાતી with ❤, related topics (one tap to explain), further reading, and Hindi/Gujarati summaries. Explain more adds deeper points. Everything is saved forever, counts toward a discovery streak, and can be saved to Notes, exported, read aloud, or discussed with the AI.

**Your genres.** Pick the fields you care about; today's three are drawn from them. **Generate today's topics with AI** asks the tutor (one tiny call, about ₹0.02, cached for the day) to invent three fresh topics inside your genres, avoiding titles already in your library; an optional toggle auto-generates when you open the page. **Library** (`#/discover/library`): every explained topic is stored in IndexedDB on the device and readable offline, with search, field and read/unread/favourite/quiz-pending filters, sorting, reading time, a distraction-free **Reading mode** with a Finished button, favourites, mark-as-read and export-all.
