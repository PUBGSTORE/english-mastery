...
async function renderHub(container) {
  const [picks, all, recs, key, est] = await Promise.all([dc.picksFor(), dc.fields(), dc.list(), ai.hasKey(), dc.estimate()]);
  ...
  mount(container, html`
    ...
    ${recs.length ? html`<h3 class="mt-lg">Your discoveries <span class="badge muted">${recs.length}</span></h3><div class="grid">${recs.map((r) => html`<a class="card dc-item" href="#/discover/${encodeURIComponent(r.id)}" style="margin:0"><div class="row between"><div class="dc-field">${r.data.emoji} ${r.data.field}</div><button class="btn btn-sm btn-ghost" data-del="${r.id}" aria-label="Delete">${icon('trash')}</button></div><strong>${r.title}</strong><div class="xs muted mt">${fmtDate(r.ts)}${r.data.quizResult ? ` · quiz ${r.data.quizResult.score}/${r.data.quizResult.total}` : ''}${r.data.teachBack ? ` · taught back ${r.data.teachBack.understanding}/10` : ''}</div></a>`)}</div>` : ''}`);
