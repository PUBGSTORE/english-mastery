...
async function renderHub(container) {
  ...
  mount(container, html`
    ...
    ${videos.length ? html`<div class="row between mt-lg"><h3 class="mb-0">Your library</h3><span class="xs muted">${videos.length} mined · ${harvested} words harvested</span></div>
      <div class="list mt">${videos.map((v) => html`<a class="list-item" href="#/mine/${encodeURIComponent(v.id)}">${v.thumb ? html`<img src="${v.thumb}" alt="" style="width:64px;height:40px;object-fit:cover;border-radius:6px;flex:none">` : icon('note')}<div class="grow"><div class="title">${v.title || v.id}</div><div class="sub">${v.channel ? v.channel + ' · ' : ''}${fmtDate(v.ts)} · ${v.stats?.density ?? '?'}% known · ${v.cardsAdded || 0} added${v.protocol?.closed ? ' · closed' : ''}</div></div>${icon('next', 'arrow')}</a>`)}</div>` : ''}
  ...
  const draw = async () => {
    mount(container, html`
      ...
      <div class="card compact"><div class="row" style="align-items:flex-start">${v.thumb ? html`<img src="${v.thumb}" alt="" style="width:120px;border-radius:8px;flex:none">` : ''}<div class="grow"><h2 style="font-size:var(--fs-lg);margin:0">${v.title}</h2><div class="xs muted">${v.channel ? v.channel + ' · ' : ''}${yt.fmtTime(v.duration)} · ${v.stats.tokens} words · ${v.stats.unknownLemmas} unknown lemmas · mined ${fmtDate(v.ts)}</div>
        ...
        ${v.url ? html`<div class="btn-row mt"><a class="btn btn-primary" href="${v.url}" target="_blank" rel="noopener" id="watch">${icon('play')} Watch on YouTube</a><a class="btn" href="#/read/${encodeURIComponent('v:' + v.id)}">${icon('book')} Reader mode</a></div>` : html`<div class="btn-row mt"><a class="btn" href="#/read/${encodeURIComponent('v:' + v.id)}">${icon('book')} Reader mode</a></div>`}</div>
