...
async function renderHub(container) {
  ...
  mount(container, html`
    ...
    ${guides.length ? html`<h3 class="mt-lg">Your library <span class="badge muted">${guides.length}</span></h3><div class="grid">${guides.map((g) => html`<a class="card owl-item" href="#/knowledge/${encodeURIComponent(g.refId)}" style="margin:0">${g.data.thumb ? html`<img src="${g.data.thumb}" alt="" style="width:100%;aspect-ratio:16/9;object-fit:cover;border-radius:10px;margin-bottom:8px">` : ''}<strong>${g.title}</strong><div class="row between mt"><div class="xs muted">${g.data.channel ? g.data.channel + ' · ' : ''}${g.data.chapters.length} chapters · ${g.data.chapters.reduce((a, c) => a + c.learnings.length, 0)} learnings · ${fmtDate(g.ts)}</div><button class="btn btn-sm btn-ghost" data-del-guide="${g.refId}" aria-label="Delete">${icon('trash')}</button></div></a>`)}</div>` : html`<div class="empty">${icon('book')}<p>Your first guide will appear here. Try a TED talk or a podcast episode you already like.</p></div>`}`);
  $('#kn-form', container).onsubmit = (e) => { e.preventDefault(); startFromUrl(container, $('#kn-url', container).value); };
  container.querySelectorAll('[data-del-guide]').forEach((b) => b.onclick = async (e) => { e.preventDefault(); e.stopPropagation(); if (await confirmDialog('Delete this guide? Its Notes page (if you saved one) stays. Re-analysing later costs tokens again.', { okLabel: 'Delete', danger: true })) { await kn.remove(b.dataset.delGuide); toast('Deleted', 'ok'); renderHub(container); } });
