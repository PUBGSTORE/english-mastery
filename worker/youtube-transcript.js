...
export default {
  async fetch(request) {
    ...
    const cacheKey = new Request(`https://transcript-cache.invalid/v/${v}/${wantLang}`);
    ...
    for (const c of attempts) {
      ...
      try {
        ...
        const payload = { videoId: v, title, channel, lang: track.languageCode, auto: !!track.kind, client: client.clientName, segments };
        try { await edge.put(cacheKey, new Response(JSON.stringify(payload), { headers: { 'Content-Type': 'application/json', 'Cache-Control': 'public, max-age=604800' } })); } catch { /* ignore */ }
        return json({ ...payload, ...(debug ? { trace } : {}) }, 200, cors);
    ...
    for (const base of PIPED) {
      try {
        ...
        const payload = { videoId: v, title, channel, lang: sub.code, auto: !!sub.autoGenerated, client: 'piped', segments };
        try { await edge.put(cacheKey, new Response(JSON.stringify(payload), { headers: { 'Content-Type': 'application/json', 'Cache-Control': 'public, max-age=604800' } })); } catch { /* ignore */ }
        return json({ ...payload, ...(debug ? { trace } : {}) }, 200, cors);
    ...
    const botBlocked = trace.some((t) => /bot|403/.test(t));
    return json({ error: noCap ? 'no captions on this video' : (botBlocked ? 'YouTube is blocking automated access right now (bot check). Wait a minute and try again.' : lastErr), title, channel, ...(debug ? { trace } : {}) }, noCap ? 404 : 502, cors);
  },
};
